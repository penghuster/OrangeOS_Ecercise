...
PRIVATE void init_descriptor(DESCRIPTOR *p_desc, u32 base, u32 limit,
        u16 attribute);
PRIVATE u32 seg2phys(u16 seg);


void memset(void *p, int zero, int size);
...
PUBLIC void exception_handler(int vec_no, int err_code, int eip, int cs, int eflags)
{
    ...
    char* err_msg[] = {"#DE Divide Error",
        ...
        "#MF x87 FPU Float-Point Error [Math Fault]",
        "#AC Alignment Check",
        "#MC Machine Check",
    ...
    //通过打印空格的方式清空屏幕的前5行，并把disp_pos 清零
    disp_pos = 0;
    for(i=0; i < 80*5; i++)
    {
        disp_str(" ");
