...
PUBLIC int do_open()
{
    ...
    if(pin)
    {
        //connects proc with file-descriptor
...
PRIVATE struct inode *create_file(char *path, int flags)
{
    ...
    printf("imap %d, smap %d\n", inode_nr, free_sect_nr);
 ...
 * alloc_smap_bit
 *
 * Allocate a bit in sectormap.
 *
 * @param dev In which device the sectormap is located
...
PRIVATE int alloc_smap_bit(int dev, int nr_sects_to_alloc)
{
    ...
    for(i = 0; i < sb->nr_smap_sects; i++)
    {
        ...
        if(free_sect_nr) //free bit found, write the bit to smap 
            WR_SECT(dev, smap_blk0_nr + i);

        if(0 == nr_sects_to_alloc)
            break;
    }

    assert(0 == nr_sects_to_alloc);
    return free_sect_nr;
}
 ...
 * @param param filename Filename of the new file 
 */ 
PRIVATE void new_dir_entry(struct inode* dir_inode, int inode_nr, char* filename)
{
    //write the dir_entry
    ...
    /*
     * including unused slots(the file has been deleted but the slot 
     * is still there)
     */ 
    int m = 0;
    ...
    for(i = 0; i < nr_dir_blks; i++)
    {
        ...
        if(m > nr_dir_entries || /*all entries has iterated or */ 
