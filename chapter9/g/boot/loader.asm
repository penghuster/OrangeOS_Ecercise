...
DispMemInfo:
    ...
    mov esi, MemChkBuf
    mov ecx, [dwMCRNumber] ;for(int i=0;i<[MCRNumber];i++)//每次得å°一个ARDS
.loop:  ;{
    mov edx, 5  ;  for(int j=0;j<5;j++)//每æ                 ¬¡得到一个ARDS中的成员
...
.1:  ;               LengthHigh             ,Type
    ...
    add eax, [dwLengthLow];
    cmp eax, [dwMemSize]  ;    if(BaseAddrLo     w + LengthLow > MemSize)
    jb .2  ;
    mov [dwMemSize], eax  ;    MemSiz                e = BaseAddrLow + LengthLow;
.2:  ;  }
    loop .loop  ;}
    ;
    call DispReturn  ;printf("\n");
    push szRAMSize  ;
    call DispStr  ;p                         rintf("RAM size:");
    add esp, 4  ;
    ;
    push dword [dwMemSize] ;
    call DispInt  ;DispInt
    add esp, 4
...
SetupPaging:
;;根据内存大小计算初始化 PDE 以及多少页表
    xor edx, edx 
    mov eax, [dwMemSize]
    mov ebx, 400000h ;;400000h = 4M = 4096 * 1024, 一个页表对应的内存大小
    div ebx 
    mov ecx, eax ;;此时商ecx即为页表的个数， 也即PDE应该的个数
    test edx, edx  ;;检查余数edx是否为0
    jz .no_remainder
    inc ecx ;;如果余数不为0就需增加一个页表
.no_remainder:
    push ecx ;;缓存页表个数，即页目录表条目数

    ;;为了简化处理，所有的线性地址对应相等的物理地址。并不考虑内存空洞
    ;;首先初始化页目录
    mov ax, SelectorFlatRW
    mov es, ax 
    mov edi, PageDirBase ;;段偏移,因段基址为0 也即为页目录表的开始地址
    mov eax, PageTblBase | PG_P | PG_USU | PG_RWW
.1:
    stosd
    add eax, 4096  ;;;为了简化让所有页表在的内存是连续的
    loop .1 
    ...
    mov edi, PageTblBase ;;此为页表所在内存的首地址
    xor eax, eax 
    mov eax, PG_P|PG_USU|PG_RWW 
.2:
    stosd
    add eax, 4096 ;;每一页指向4K空间
    loop .2
...
;;将Kernel.bin 的内容经过整理对齐后放到新的位置，遍历每一个 Program Header，
;;根据 Program Header中的信息来确定把什么放进内存，放到什么位置，以及放多少
InitKernel:
    xor esi, esi 
    mov cx, word [BaseOfKernelFilePhyAddr + 2Ch]  ;;ecx <- pELFEHdr->e_phnum
    movzx ecx, cx ;; 将cx中的值取出填充于 ecx,位数不够用0填充
