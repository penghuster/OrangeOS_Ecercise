 ...
 *      four 16-byte partitions entries. Should end with 55h & AAh 
 *
...
{   
     ...
     *  bits 6-0 are zero (when not zero this byte is also the drive number of the 
     *  drive is also the drive number of the drive to boot so the active partition 
     *  is always found on drive 80H, the first hard disk).
     */
    u8 boot_ind;
    ...
    /*
     * Starting Sector.
     *  only bits 0-5 are used. bits 6-7 are the upper two bits for the 
    ...
    /*
     * Starting Cylinder.
     *  This field contains the lower 8 bits of the cylinder value.
     *  starting cylinder is thus a 10-bit number, with a maximum value of 1023
     */ 
    u8 start_cyl;
    ...
    /*
     * starting sector counting from 0 Relative sector /start in LBA 
     */ 
    u32 start_sect;
